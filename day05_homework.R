...
answer_1 <- flights %>% left_join(planes, by = "tailnum")
...
# airplane in `planes`?
answer_3 <- !any(is.na(answer_1$manufacturer))
...
answer_4 <- flights %>% left_join(airlines, by = "carrier")
...
answer_5 <- answer_4 %>% group_by(name) %>% summarize(avg_arr_delay = mean(arr_delay, na.rm = TRUE)) %>% arrange(desc(avg_arr_delay))
